...
### Added
- ESLint (flat config, v9) and Prettier in both `backend/` and `frontend/`. Shared style: 100-col, single quotes, trailing commas, semicolons, 2-space indent. npm scripts: `lint`, `lint:fix`, `format`, `format:check`.
- `AUTH_PROVIDER=dev` option documented in `.env.example`.

### Changed
- Applied prettier + eslint autofix across the entire codebase. No runtime behaviour change.
- Removed redundant `hasSession` alias in Sidebar, BottomNav, Workouts — callers now read `hasDraft` from `useDraft()` directly.
- Removed dead `isExerciseUsed` callback in `WorkoutChat` (replaced inline during the 0.5.0 refactor but left behind).
